...
#
# Layout:  %LOCALAPPDATA%\Programs\LogicalLunge   app (package app\ folder + downloaded tools)
...
try {
    ...
    # Generated by the core on first run (Super menu app list): keep it across updates
    $apps = Join-Path "$RB\app" 'ui\logical-lunge\apps.json'
    ...
    # ------------------------------------------------------------ data of 0.1.x
    if ($legacy -or (Test-Path $OLD_STATE)) {
        ...
        foreach ($d in 'tools\wezterm', 'tools\bin') {
            $from = Join-Path $OLD_LL $d; $to = Join-Path $APP $d
    ...
    # The core is the root of the desktop: it starts the window manager and the shell as its own children
    Register-LLTask 'Start' (New-ScheduledTaskAction -Execute (Join-Path $APP 'lunge.exe') -WorkingDirectory $UserProfile) $trigger $principalUser $forever
    if (-not $NoSensors) {
    ...
    foreach ($p in $APP, $DATA, $CONF) { & icacls $p /setowner $UserName /T /C /Q | Out-Null }
    Assert-NotCancelled
    $ok = $true
}
